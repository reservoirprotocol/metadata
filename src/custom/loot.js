...
export const fetchContractTokens = (_chainId, contract, continuation) => {
  const minTokenId = continuation ? Math.max(continuation, 1) : 1;
  const maxTokenId = continuation ? Math.min(continuation + 1000, 7881) : 7881;
  ...
  return {
    continuation: maxTokenId === 7881 ? undefined : maxTokenId + 1,
    metadata,
  };
};
